const fs = require('fs');
const role = 'xxx'; //ID roli obywatelka
...
module.exports = {
    name: 'wlw',
    description: 'Dodaj rangę "Obywatelka" dla danego użytkownika (tylko dla administracji)',
    execute(message) {
		...
		if (channelid === channel) {
			if (message.member.roles.cache.some(role => role.id === needRole)){
				...
				if (user.roles.cache.some(r => r.id === role)) {
					message.reply("Usunięto rolę OBYWATELKA gracza " + ping);
					user.roles.add(oldRole);
					user.roles.remove(role);
				} else {
					message.reply("Dodano rolę OBYWATELKA dla gracza " + ping);
					user.roles.add(role);
			...
			message.delete();
			author.send("Na tym kanale nie możesz użyć komendy /wlw!");
		}
    },
};